...
let stringFromCharList (cl : List<char>) =
...
[<Fact>]
let ``part1`` () =
    let grid = File.ReadAllLines "2024/Day4/Data.txt" |> Array.map _.ToCharArray() |> Grid.create
    ...
    let res =
        ...
        |> Array.mapi (fun rowIndex row ->
            ...
            |> Array.mapi (fun colIndex cell ->
                ...
                if cell = 'A' then
                    let offsetValues = pos |> Pos.getDiagonalOffsets |> List.map (fun offset -> grid |> Grid.tryGet offset ' ')
                    ...
                    if (x1 = "MS" || x1 = "SM") && (x2 = "MS" || x2 = "SM") then 1 else 0
                else
                    0
            )
            |> Array.sum
