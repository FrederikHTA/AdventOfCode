...
module Grid =
    ...
    let get (grid: Grid<'T>) (pos: Pos) : 'T = grid.[pos.X].[pos.Y]

    let set (grid: Grid<'T>) (pos: Pos) (value: 'T) : unit = grid.[pos.X].[pos.Y] <- value

    let containsPos (pos: Pos) (grid: Grid<'T>) : bool =
        pos.X >= 0
        && pos.X < Array.length grid
        && pos.Y >= 0
        && pos.Y < Array.length grid.[0]

    let tryGet (pos: Pos) (defaultValue: 'T) (grid: Grid<'T>) : 'T =
        if grid |> containsPos pos then
            grid.[pos.X].[pos.Y]
        else
            defaultValue
