...
object Day7 {
  ...
  def part1(nodes: Map[String, Node]) = {
    val directories = nodes.filter(x =>
      x._2 match {
    ...
    val sums = directories.map(key =>
      nodes.view
        ...
        .map(x => x.asInstanceOf[File].size)
        .sum
    )
  ...
  def parseInput(input: Seq[String]) = {
    val emptyDirectory = DirectoryContext("", "", Map.empty[String, Node])
    input.foldLeft(emptyDirectory)(buildTree).nodes
  }
